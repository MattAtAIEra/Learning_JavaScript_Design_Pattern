...
```
skills/
├── 00 아키텍트의 의사결정 흐름        ← 여기서 시작: 전체 지도
│
├── 01 모듈 & 네임스페이스             ┐
├── 02 객체 생성                       │
│      (Factory, Builder, Singleton)   │  기초 레이어
├── 03 공유 유틸리티 &                 │  (순서 무관)
│      함수형 코어                     │
├── 04 IO & 인프라 Adapter            │
├── 05 횡단 관심사                     ┘
│
├── 06 의존성 주입 & IoC 컨테이너      ← 핵심: 01-05를 연결
│
├── 07 컴포넌트 간 통신                ┐
├── 08 상태 & 비즈니스 로직            │  애플리케이션 레이어
├── 09 MVC / MVP / MVVM              │  (순서 무관)
├── 10 비동기 & 회복탄력성             │
├── 11 테스트 전략                     ┘
│
└── 12 팀 & 프레임워크 통합            ← 마무리: 팀 컨벤션
```
