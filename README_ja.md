...
このリポジトリにある15本のスキルファイルが、そのガイドだ。

## 中身

15本のスキルファイルを、**レイヤードアーキテクチャ**に沿って配置：
...
**これは、Vibeコーディングを始めたばかりの人がひとりで気づけることではない。でも15本のスキルファイルがあれば、あなたのAIエージェントはベテランエンジニアが何年もかけて体得したことを即座に取り込み ── 次のコミットから実践できる。**

## ライセンス

SKILL.MDの教育コンテンツはオリジナルの著作物です。コード例は *Mastering JavaScript Design Patterns, Second Edition*（Simon Timms, Packt）および *Learning JavaScript Design Patterns*（Addy Osmani, O'Reilly）を参考にしています。書籍のソースコードおよびPDFは本リポジトリに含まれていません。
